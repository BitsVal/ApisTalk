...
public enum ApisProtoVersion {
    ...
    private static final ApisProtoVersion[] VALUES;

    static {
        // this prevent values to be assigned with the wrong order
        // and ensure valueOf to work fine
        final ApisProtoVersion[] values = values();
        VALUES = new ApisProtoVersion[values.length + 1];
        for (ApisProtoVersion mqttMessageType : values) {
    ...
    ApisProtoVersion(int value) {
        this.value = value;
    }

    public static ApisProtoVersion valueOf(int type) {
