 ...
 **/
public class MsgDataProtocolHandler extends AbstractSystemProtocolHandler {
    private final Logger logger = LoggerFactory.getLogger(MsgDataProtocolHandler.class);
    ...
    @Override
    CMD command() {
        return CMD.CMD_INSTANT_MESSAGING_DATA_REQ;
    }
    ...
    public void handler(Message<ProtocolPackage> protocolMsg) throws Exception {
        ...
        if(HandlerManger.hasHandler(HandlerType.IM_STORE)){
            vertx.eventBus().<ProtocolPackage>request(HandlerType.IM_STORE.getAddress(),protocolMsg.body(),rsp->{
