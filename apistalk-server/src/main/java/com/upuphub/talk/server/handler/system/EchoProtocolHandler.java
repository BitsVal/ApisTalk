 ...
 **/
public class EchoProtocolHandler extends AbstractSystemProtocolHandler {
    private final Logger logger = LoggerFactory.getLogger(EchoProtocolHandler.class);
    ...
    public void handler(Message<ProtocolPackage> protocolMsg) throws Exception {
        ...
        if(Environment.isRegistered(protocolReq.getHeader().getFrom())){
            Protocol protocolRsp = ProtocolFactory.buildEchoRsp(protocolReq);
