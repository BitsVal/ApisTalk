 ...
 **/
public class QoSReceivedProtocolHandler extends AbstractSystemProtocolHandler {
    private final Logger logger = LoggerFactory.getLogger(QoSReceivedProtocolHandler.class);
    ...
    @Override
    CMD command() {
        return CMD.CMD_QOS_RECEIVED_REQ;
    }
    ...
    public void handler(Message<ProtocolPackage> protocolMsg) throws Exception {
        ...
        if(Environment.isRegistered(protocolReq.getHeader().getFrom())){
           Protocol protocolRsp = messageRecived(protocolReq);
    ...
    private Protocol messageRecived(Protocol protocolReq){
        Protocol protocolRsp = ProtocolFactory.buildAuthorizationRsp(
